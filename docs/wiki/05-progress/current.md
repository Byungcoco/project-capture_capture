---
title: Current
type: progress
status: verified
tags: [current-scope, vertical-slice]
updated: 2026-08-14
summary: 지금 참인 상태, 다음 목표와 완료 조건만 유지하는 작업 원본.
---

# Current

## 현재 상태

- 설계 확정, 저장소 세팅 완료. 브라우저 데모로 캡처 파이프라인 개념 검증 완료.
- 마일스톤 1 완료: Vite+TS 뼈대, 고정 타임스텝 루프, 직교 카메라 씬에 박스 지형 렌더 동작.
- 마일스톤 2 구현 완료: 관성형 이동·점프와 박스 지형 2D AABB 충돌 동작. 공중 수평 가속 40%, 공중 무입력 감속 초당 8, 지상 감속 초당 36으로 조정하고 조작감 재확인 대기.

## 다음 목표 — 마일스톤 2 조작감 확인

A/D 관성 이동, Space 점프, 공중 방향 전환과 박스의 바닥·양쪽 벽·천장 충돌을 브라우저에서 확인한다. 낮춘 공중 제어의 조작감 승인 후 이번 조정을 커밋하고 마일스톤 3으로 이동한다.

남은 마일스톤 (각 단계마다 브라우저 확인 후 커밋):

- 3. 카메라 회전(Q/E/F) + 프레임 오버레이 + 캡처 파이프라인 (스탬프 디버그 표시)
- 4. 붙여넣기: 스냅 배치, 슬래브 지오메트리 + 폴리곤 콜라이더, 플레이어가 밟기 가능
- 5. 골 판정 + 리셋 + 캡처 횟수 카운터

## 완료 조건

- pnpm dev 로컬 실행, pnpm build 성공, 새로고침 후 재현 가능, 콘솔 에러 0
- core/capture/stamp 모듈이 Three.js·DOM을 import하지 않음
- 같은 입력 시퀀스 재생 시 같은 결과 (결정론 스모크 테스트)

## 이번 범위에서 제외

속성 시스템, 다중 스테이지, 리플레이 저장, 아트, 사운드, 메뉴 UI
