...
export function createAabbCollisionWorld(
    ...
    moveAabb(positionValue, velocityValue, halfSize, stepSeconds): CollisionMoveResult {
      ...
      for (const axis of ['x', 'y', 'z'] as const) {
        ...
        for (const collider of colliders) {
          if (!overlaps(position, halfSize, collider) || delta === 0) continue
          position[axis] = delta > 0
            ? collider.center[axis] - collider.halfSize[axis] - halfSize[axis]
            : collider.center[axis] + collider.halfSize[axis] + halfSize[axis]
          velocity[axis] = 0
          if (axis === 'y' && delta < 0) grounded = true
          if (axis === 'x' || axis === 'z') blockedHorizontally = true
        }
      }
      return { position, velocity, grounded, blockedHorizontally }
    },
